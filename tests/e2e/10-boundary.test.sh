...
suite "broker is unreachable from lab"
# Not on the `lab` network at all: Docker DNS should not even resolve it.
out=$(lab_sh 'getent hosts broker || echo NXDOMAIN')
check_contains "broker does not resolve from lab" "$out" "NXDOMAIN"

out=$(lab_sh 'curl -s --max-time 3 -o /dev/null -w "%{http_code}" http://broker:8080/healthz --noproxy "*" || echo CONNFAIL')
check_contains "direct connection to broker fails" "$out" "CONNFAIL"

# The proxy bridges `lab` and `secure`, so it is the one path that could reach
# the broker. 000_policy.py has to close it.
check "proxy refuses to tunnel to broker" "403" \
  "$(lab_code "http://broker:8080/github/token" --proxy http://proxy:8080)"
check "proxy refuses to tunnel to broker with a spoofed Host" "403" \
  "$(lab_code "http://broker:8080/github/token" --proxy http://proxy:8080 -H "Host: api.github.com")"
check "proxy refuses to tunnel to cred-gateway" "403" \
  "$(lab_code "http://cred-gateway/github/token" --proxy http://proxy:8080)"

suite "cred-gateway exposes only the whitelisted endpoints"
check "/healthz reachable" "200" "$(lab_code "http://cred-gateway/healthz")"
...
check "ANTHROPIC_API_KEY is still the placeholder" "proxy-injected" \
  "$(lab_sh 'printf %s "$ANTHROPIC_API_KEY"')"
check "GH_TOKEN is still the placeholder" "proxy-injected" \
  "$(lab_sh 'printf %s "$GH_TOKEN"')"

# Nothing under the lab container's home should look like a vendor secret.
...
env_dump=$(lab_sh 'env' )
check_no_secret "no credential-shaped value in lab's environment" "$env_dump" "${SECRET_PATTERNS[@]}"

git_cfg=$(lab_sh 'cat ~/.gitconfig 2>/dev/null || true')
