...
ident=$(lab_body "http://cred-gateway/github/identity")
...
# Branch count, not `.size`: that field is the repository size in KB and a repo
# holding one README rounds to 0, which would skip this suite on a perfectly
# good target. An empty repo is one with no branches.
branches=$(lab_sh "gh api repos/$REPO/branches --jq 'length' 2>/dev/null")
if [ "${branches:-0}" = "0" ]; then
  suite "clone and push"
...
cleanup_branch() {
  lab_sh "cd /tmp/e2e-clone 2>/dev/null && git push origin --delete '$BRANCH' >/dev/null 2>&1 || true" >/dev/null
}
...
out=$(lab_sh "rm -rf /tmp/e2e-clone && git clone --depth 1 https://github.com/$REPO /tmp/e2e-clone 2>&1")
if lab_sh 'test -d /tmp/e2e-clone/.git && echo yes' | grep -q yes; then
  ok "clone succeeded"
else
...
cfg=$(lab_sh 'cat /tmp/e2e-clone/.git/config 2>/dev/null || true')
...
if printf '%s' "$out" | grep -q PUSH-OK; then
  remote=$(lab_sh "gh api repos/$REPO/branches/$BRANCH --jq .name 2>/dev/null")
  check "branch exists on the remote" "$BRANCH" "$remote"

  author=$(lab_sh "cd /tmp/e2e-clone && git log -1 --format=%ae")
  check "commit is authored by the App identity" "$email" "$author"
fi

suite "cleanup"
cleanup_branch
gone=$(lab_sh "gh api repos/$REPO/branches/$BRANCH >/dev/null 2>&1 && echo STILL-THERE || echo gone")
check "scratch branch removed" "gone" "$gone"
lab_sh 'rm -rf /tmp/e2e-clone' >/dev/null
