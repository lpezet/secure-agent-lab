...
| `06-check-invariants` | no | `scripts/check-invariants.sh` against synthetic deployments: the two leaks from the incident report that opened #26, one planted violation per fail-severity check, a multi-tenant injection wildcard failing while a single-tenant one only notes, the `000_policy.py` pretty_host exemption not leaking to other files, note-severity findings not failing the run, and structural assertions that the lint and the scanner share one implementation |
| `07-hostmatch` | no (needs `python3`) | Unit tests for `stack/proxy/hostmatch.py`: `evilexample.com` and the bare apex do not match `*.example.com`, normalisation of case / trailing root dot / `:port` / bracketed IPv6, `find()` returning which pattern matched, longest suffix winning regardless of list order, and uninterpretable patterns being skipped rather than raised on |
| `08-credential-principal` | no | `--secrets-dir`: a GitHub PAT and a bare `authorized_user` ADC are failures, an App key and both machine ADC shapes are silent, Anthropic's absent machine identity and an unrecognised shape are notes rather than passes, the class is reported as *not run* without the flag, symlinks out of the directory are not followed — and, against deliberately credential-shaped fixtures, that no credential value ever reaches stdout |
...
| `45-broker-github-scope` | yes | The real `github.js` against a TLS stub standing in as `api.github.com` (throwaway RSA key, `--add-host`): `token_issued` and `credential_issued` name the `permissions` and `repository_selection` the token carries, no token value or key material reaches the trail, repository names stay out of it, and an unreachable installation endpoint degrades the trail to `unknown` rather than failing the mint |
| `46-broker-gcp` | yes | The real `gcp.js` against a TLS stub standing in for Google: a `service_account` key file signing a real RS256 JWT-bearer assertion, the configured SA cross-checked against the file, a bare `authorized_user` refused as the operator's own identity, `external_account` refused as unimplemented, and no key material or token in the trail |
