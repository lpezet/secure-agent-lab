...
**An addon must not act on a request an earlier addon has already refused.** mitmproxy calls every addon's `request` hook regardless of whether the flow has been answered, so `if flow.response is not None: return` is the first line of every addon that acts on a request. Without it, `001_allowlist.py` overwrote `000_policy.py`'s refusal — recording `reason=allowlist` for an agent probing the credential broker — and an injection addon running after a denial fetched a credential from the broker and logged `cred_injected` for a request that never left. Neither is a boundary failure; both are the audit trail claiming something that did not happen, and the second claims a credential was spent. `000_policy.py` is exempt: it decides first by construction and must never stand aside. The guard is deliberately dependency-free, because deployments vendor these files at pins that may predate any shared helper. `inv_response_guard` enforces it; `tests/integration/20` and `25` cover both runtime symptoms (#87).
